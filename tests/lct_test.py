...
def test_lct_signal():
    ...
    dus, all_fvals, all_uvals = _get_vals()
...
def test_lct_fourier():
    ...
    _ndiff_files(
        ...
        pkio.write_text(work_dir.join("fourier_actual.txt"),
            r
        ),
        work_dir.join("ndiff.out"),
        data_dir
...
def test_chirp_multiply():
    ...
    _ndiff_files(
        ...
        pkio.write_text(work_dir.join("cm_signals_actual.txt"),
            r
        ),
        work_dir.join("ndiff.out"),
        data_dir
...
def test_lct_decomposition():
    ...
    ms = [[[ 0.5, -0.5], [ 0.5,  1.5]],
         [[ 2.,   1. ], [-2.6, -0.8]],
         [[-0.5,  0.5], [-0.5, -1.5]]]
    d = str([lct.lct_decomposition(m) for m in ms])
    _ndiff_files(
        data_dir.join("lct_decomp.txt"),
        pkio.write_text(work_dir.join("lct_decomp_actual.txt"),
            d
        ),
        work_dir.join("ndiff.out"),
        data_dir
...
def _cast_from_complex_signal(signals, signal_function, factor):
        if factor:
            ...
            s = [signal_function(sig) for sig in signals]
        for arr in s:
...
def _fn2(u):
    return 1.5 * _tri(u / 3.) - 0.5 * _tri(u)
